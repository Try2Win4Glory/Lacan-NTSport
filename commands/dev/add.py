...
class Command(commands.Cog):
    ...
    async def add(self, ctx, userid, amount):
        ...
        try:
            ...
            userpoints = int(amount)
            dbclient.create_doc(collection, {'userid': str(userid), 'points': int(amount)})
        embed = Embed('Success!', f'<@{ctx.author.id}> just added **{amount}** <:Lacan:766669740545540097> to <@{userid}>\'s balance. \n<@{userid}> now has **{str(userpoints)}** <:Lacan:766669740545540097>!', 'white_check_mark')
