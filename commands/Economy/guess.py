            ...
            if response.content.lower() in list('abcd'):
                if response.content.lower() == guesser.correct:
                    ...
                    try:
                        ...
                        await dbclient.create_doc(collection, {'userid': str(ctx.author.id), 'points': 2})
                    #Embed for lacan Log
                    ...
                    embed = Embed('<a:false:800330847865143327>  Wrong!',f'Your answer was wrong! The correct answer was **{guesser.options[guesser.correct]}**. You also lost **2** '+random_lacan+'.')
                    await embed.send(ctx)
                    dbclient = DBClient()
                    collection = dbclient.db.pointsdb
                    data = await dbclient.get_array(collection, {'$and': [{'userid': str(ctx.author.id)}, {'userid': str(ctx.author.id)}]})
